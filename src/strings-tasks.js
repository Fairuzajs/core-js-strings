...
function formatTime(minutes, seconds) {
  const minut = minutes < 10 ? `${minutes}`.padStart(2, '0') : minutes;
  const second = seconds < 10 ? `${seconds}`.padStart(2, '0') : seconds;
  const times = `${minut}:${second}`;
  return times;
}
...
function countVowels(str) {
  const m = str.match(/[aeiouyAEIOUY]/gi);
...
function findLongestWord(sentence) {
  ...
  let longword = arr[0];
  arr.forEach(function findLong(item) {
